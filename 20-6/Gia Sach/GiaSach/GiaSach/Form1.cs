...
namespace GiaSach
{
    public partial class Form1 : Form
    {
        ...
        public static DataTable tb_muontra = new DataTable();
        public static DataTable ls_muontra = new DataTable();
        ...
        public void tbs_columns()
        {
            ...
            tb_columns(grid_tb_muontra, tb_muontra);

            //
            data_tb("danhmuc.txt", tb_danhmuc);
            data_tb("hientai.txt", tb_hientai);
            data_tb("ls.txt", tb_ls);
            data_tb("tb_muontra.txt", tb_muontra);

            for (int i = 0; i < tb_danhmuc.Rows.Count; i++)
            {
                tb_danhmuc.Rows[i]["Số dư"] = Convert.ToInt32(tb_danhmuc.Rows[i]["Số lượng"].ToString()) - Convert.ToInt32(tb_danhmuc.Rows[i]["Số lượng mượn"].ToString()); 
            }

            // tb_lịch sử mượn trả 
            ls_muontra.Columns.Add("Ngày");
            ls_muontra.Columns.Add("Giao dịch");
            ls_muontra.Columns.Add("Mã phiếu");
            ls_muontra.Columns.Add("Mã bạn đọc");
            ls_muontra.Columns.Add("Tên bạn đọc");
            ls_muontra.Columns.Add("Mã nhân viên");
            ls_muontra.Columns.Add("Mã sách");
            ls_muontra.Columns.Add("Tên sách");
            ls_muontra.Columns.Add("Thể loại");
            ls_muontra.Columns.Add("Nhà xuất bản");
            ls_muontra.Columns.Add("Vị trí");
            ls_muontra.Columns.Add("Số lượng mượn");
            ls_muontra.Columns.Add("Ngày mượn");
            ls_muontra.Columns.Add("Hạn mượn");
            ls_muontra.Columns.Add("Ngày trả");
            ls_muontra.Columns.Add("Số lượng trả");
            ls_muontra.Columns.Add("Ghi chú");
            data_tb("ls_muontra.txt", ls_muontra);
        }
        ...
        public static void show_grid(DataGridView dg, DataTable tb)
        {
            ...
            dg.ColumnHeadersDefaultCellStyle.BackColor = Color.Yellow;
            dg.Rows.Clear();
        ...
        private void panelke_Click(object sender, EventArgs e)
        {
            ...
            void pa(int x, int y, Panel tu)
            {
                ...
                for (int j = 0; j < 5; j++)
                {
                    for (int i = 0; i < 2; i++)
                    {
                        ...
                        l.Location = new Point(53, 37);
                        //l.Dock = DockStyle.Bottom; 
                        l.Font = new System.Drawing.Font("SMicrosoft Sans Serif", 12.75F, System.Drawing.FontStyle.Bold, System.Drawing.GraphicsUnit.Point, ((byte)(0)));
                        l.ForeColor = Color.Red;
                        l.BackColor = SystemColors.GradientInactiveCaption;
                        p1.Controls.Add(l);
                        l.Size = new Size(44, 20);
                        l_o.Add(p1);
            ...
            for (int i = 0; i < tb_hientai.Rows.Count; i++)
            {
                ...
                for (int j = 0; j < l_o.Count; j++)
                {
                    ...
                    {
                        l_o[j].BackgroundImage = global::GiaSach.Properties.Resources.Untitled14;
           ...
           // float x = ((float)ton / tong) * 100;
            float dungluong = 100 * ((float)dung_luong / 20) ;

            double tile = (dungluong / 100) * 160;
            pndungluongtu.Size = new Size((int)tile,pndungluongtu.Height);
            string adl = dungluong.ToString();
            lb_trangthai.Text = dungluong.ToString("##,0.0") + "%";
            lb_trangthai.BringToFront(); 
        ...
        // dung lượng 
        public void dungluong()
        {
            int tong = 100 * (20* Convert.ToInt32(tb_cauhinh.Rows[0][0].ToString()) + Convert.ToInt32(tb_cauhinh.Rows[0][1].ToString())); // 100* so ngan  
            int ton = 0;
            for (int i = 0; i < tb_hientai.Rows.Count; i++)
            {
                ton += Convert.ToInt32(tb_hientai.Rows[i]["Số lượng"]); // so ngan da dung 
            }

            float x = ((float)ton / tong) * 100;
            circularProgressBar1.Maximum = tong;
            circularProgressBar1.Value = ton;
            circularProgressBar1.Text = x.ToString("##,0.0");
        ...
        private void btLoc_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                addItem(lit_loc(tb_hientai, 6), cbb_vt_6);
                pnSearchDanhm.Visible = true;
                pnSearchDanhm.Location = new Point(476, 0);
                spnSearchHt.Visible = false;
            }
            ...
            List<string> l(DataTable tb, int c)
            {
                ...
                for (int i = 0; i < lds.Count; i++)
                {
                    ...
                    for (int j = 0; j < tb.Rows.Count; j++)
                    {
                        ...
                        {
                            sl += Convert.ToInt32(tb.Rows[j][7].ToString());
                            tb1.Rows[i][1] = sl;
                        }
                    }
                }
        ...
        public void radio(Panel p, List<string> lt)
        {
            for (int i = 0; i < lt.Count; i++)
            {
                ...
                b.TabIndex = i;
                b.Text = lt[i];
                b.ForeColor = System.Drawing.SystemColors.Highlight;
        ...
        private void hoanthanh_Click(object sender, EventArgs e)
        {
            ...
            if (l_pds_home.Count == 0)
            {
                ...
                F_Cauhinh.tool_save_txt(tb_muontra, "tb_muontra.txt");
                F_Cauhinh.tool_save_txt(ls_muontra, "ls_muontra.txt");
                // thông tin 
        ...
        {
            pnls_Nhap_xuat.Visible = true;
            pnls_Muon_Tra.Visible = false;
            // add item 
        ...
        {
            pnls_Nhap_xuat.Visible = false;
            pnls_Muon_Tra.Visible = true;
            show_ls();
        }
        ...
        {
            lbName.Text = "BẢNG DANH SÁCH MƯỢN TRẢ";
            grid_hientai.Visible = false;
            grid_danhmuc.Visible = false;
            grid_tb_muontra.Visible = true;
            button5.Visible = false;

            for (int i = 0; i < tb_muontra.Rows.Count; i++)
            {
                TimeSpan ts = DateTime.Parse(tb_muontra.Rows[i]["Hạn mượn"].ToString()) - DateTime.Now;

                tb_muontra.Rows[i]["Ghi chú"] = "Còn " + ts.Days.ToString() + " ngày";
                if (ts.Days < 0)
                {
                    tb_muontra.Rows[i]["Ghi chú"] = "Quá hạn " + (-ts).Days.ToString() + " ngày";
                }
            }
            show_grid(grid_tb_muontra, tb_muontra);

        }

        // show  lịch sử mượn trả : Tất cả các mã phiếu
        public void show_ls()
        {
            DataTable tb = new DataTable();
            tb_columns(Grid_maphieu, tb);  // tên cột 

            // 



            List<string> l_mp_muon = new List<string>();
            List<string> l_mp_tra = new List<string>();
            for (int i = 0; i < ls_muontra.Rows.Count; i++)
            {
                if (ls_muontra.Rows[i][1].ToString() == "Mượn sách")
                {
                    l_mp_muon.Add(ls_muontra.Rows[i]["Mã phiếu"].ToString());
                }

                if (ls_muontra.Rows[i][1].ToString() == "Trả sách")
                {
                    l_mp_tra.Add(ls_muontra.Rows[i]["Mã phiếu"].ToString());
                }
            }
            l_mp_muon = l_mp_muon.Distinct().ToList();
            l_mp_tra = l_mp_tra.Distinct().ToList();

            for (int i = 0; i < l_mp_muon.Count; i++)
            {
                tb.Rows.Add("", "Mượn sách", l_mp_muon[i], "", "", "", "", "", "", "");
            }
            for (int i = 0; i < l_mp_tra.Count; i++)
            {
                tb.Rows.Add("", "Trả sách", l_mp_tra[i], "", "", "", "", "", "", "");
            }

            for (int i = 0; i < tb.Rows.Count; i++)
            {
                string giaodich = tb.Rows[i][1].ToString().Trim() + tb.Rows[i][2].ToString().Trim();
                int dem = 1;
                int sl = 0;
                for (int j = 0; j < ls_muontra.Rows.Count; j++)
                {
                    string giaodichss = ls_muontra.Rows[j][1].ToString().Trim() + ls_muontra.Rows[j]["Mã phiếu"].ToString().Trim();

                    if (string.Compare(giaodich, giaodichss, true) == 0)
                    {
                        dem++;
                        if (ls_muontra.Rows[j][1].ToString().Trim() == "Mượn sách")
                        {
                            sl += Convert.ToInt32(ls_muontra.Rows[j]["Số lượng mượn"].ToString());
                        }
                        else
                        {
                            sl += Convert.ToInt32(ls_muontra.Rows[j]["Số lượng trả"].ToString());
                        }


                        // 
                        tb.Rows[i][0] = ls_muontra.Rows[j][0];                 // ngày          0
                        tb.Rows[i][1] = tb.Rows[i][1].ToString().Trim();                             // giao dịch     1
                        tb.Rows[i][2] = tb.Rows[i][2].ToString().Trim();                                      // mã phiếu      2
                        tb.Rows[i][3] = ls_muontra.Rows[j]["Mã bạn đọc"];        // mã bạn đọc    3
                        tb.Rows[i][4] = ls_muontra.Rows[j]["Mã nhân viên"];      // mã nhân viên  5
                        tb.Rows[i][5] = dem;                                     // đầu sách      6
                        tb.Rows[i][6] = sl;                                      // tổng số lượng 7
                        tb.Rows[i][7] = ls_muontra.Rows[j]["Ngày mượn"];         // ngày mượn     8
                        tb.Rows[i][8] = ls_muontra.Rows[j]["Hạn mượn"];         // ngày trả      9
                        tb.Rows[i][9] = ls_muontra.Rows[j]["Ghi chú"];             // ghi chú       10 
                    }
                }
            }

            show_grid(Grid_maphieu, tb);
        }

        int index_mp = -1;
        private void Grid_maphieu_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            index_mp = e.RowIndex;

            // combobox 
            addItem(lit_loc(ls_muontra, 2), cbb_maphieu); // mã phiếu 
            addItem(lit_loc(ls_muontra, 3), cbb_madocgia);
            addItem(lit_loc(ls_muontra, 4), cbb_tendocgia);
            addItem(lit_loc(ls_muontra, 5), cbb_nhanvien);

            // show grid 
            DataTable tb = new DataTable();
            for (int i = 0; i < ls_muontra.Columns.Count; i++)
            {
                tb.Columns.Add(ls_muontra.Columns[i].Caption);
            }

            if (index_mp > 0)
            {
                for (int i = ls_muontra.Rows.Count - 1; i >= 0; i--)
                {
                    string magrid = Grid_maphieu.Rows[index_mp].Cells[1].Value.ToString() + Grid_maphieu.Rows[index_mp].Cells[2].Value.ToString();
                    string sosanh = ls_muontra.Rows[i][1].ToString() + ls_muontra.Rows[i][2].ToString();
                    if (string.Compare(magrid, sosanh, true) == 0)
                    {
                        DataRow dr = ls_muontra.Rows[i];
                        tb.Rows.Add(dr.ItemArray);
                    }
                }
                grid_maphieu_chitiet.DataSource = null;
                grid_maphieu_chitiet.DataSource = tb;
            }
        }
        ...
        + lịch sử nhập xuất, cho mượn trả ??

        - quản lý bạn đọc
       
        + thông tin bạn độc / danh sách độc giả 
        + thông báo quá hạn  / lịch sử mượn trả 
         */
        #endregion
        ...
        private void bt_nhanvien_Click(object sender, EventArgs e)
        {
            ...
            pn_bandoc.Visible = false;

            // danh sách nhân viên 
            grid_nhanvien.DataSource = FLogin.Table_Acount;
            editGridview(grid_nhanvien); 
        }

        private void bt_bandoc_Click(object sender, EventArgs e)
        {
            pn_nhanvien.Visible = false;
            pn_bandoc.Visible = true;
