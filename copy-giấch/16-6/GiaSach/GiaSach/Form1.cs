...
namespace GiaSach
{
    public partial class Form1 : Form
    {
        ...
        public void timkiem(DataTable tb, DataGridView dg, TextBox txb)
        {
            dg.Rows.Clear();
            int dem = 0;
        ...
        private void bt_timkiemdm_Click(object sender, EventArgs e)
        {
            if (lbName.Text != "" && lbName.Text.Trim() != "Tìm kiếm")
            {
                if (lbName.Text == "BẢNG DANH MỤC SÁCH")
                {
                    timkiem(tb_danhmuc, grid_danhmuc, tbSearchHt);
                }
                // 
                if (lbName.Text != "BẢNG DANH MỤC SÁCH")
                {
                    timkiem(tb_hientai, grid_hientai, tbSearchHt);
                }
        ...
        {
            timkiem(tb_ls, grid_ls_nx, tbSearchLs); // lich su nhap xuat 
        }
